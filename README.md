...
___
 ...
 ## **Устанавливаем пакет `git` и скачиваем репозиторий**

```
apt-get install git
git clone ‘https://github.com/Dustiest7/exam-2024.git’
```

**Переходим в папку `exam-2024`**
...
**Архив успешно распакован, после этого переходим в директорию с необходимой виртуальной машиной**

**Для всех ВМ прописываем:**
```
chmod +x hq*
chmod +x br*
chmod +x cli*
...
> [!IMPORTANT]
> Нулевой скрипт обновит полностью систему, при необходимости надо закомментировать вторую строку.
> Также везде закомментируем строку с пакетом `rsyslog`, он нам не понадобится.

**Нулевой скрипт устанавливает необходимые пакеты**
```
./hq-r-0.sh
./br-r-0.sh
./hq-srv-0.sh
...
**Первый скрипт конфигурирует интерфейсы, в нём изменяем имена интерфейсов, для просмотра интерфейсов на `br*` используем скрипт `nmcli.sh`**

```
sed -i 's/ens160/ens33/g' имя_скрипта
sed -i 's/ens224/ens35/g' имя_скрипта
sed -i 's/ens256/ens36/g' имя_скрипта
```


 ## **IP-адресация**

![Screenshot_4](https://github.com/Dustiest7/exam-2024/assets/169822164/430b1247-001f-49a2-8f56-ff74c5860978)

___

 ## **Настройка GRE-туннеля**

**На HQ-R, в папке `tunel/options` меняем имя интрерфейса на `ens35`**
...
**Быстрое комментирование строк в `vim`**
```
ctrl + V
стрелками выделяем нужные строки
shift + I
вводим символ #
esc
```

**В папке `dhcp`, в файле `dhcpd` и `dhcpd6` меняем имя интерфейса, на `ens35`**</br>
**В файле `dhcpd.conf` меняем `mac-адрес`**</br>
**Из папки `exam-2024` копируем файл `dhcpd6.conf` и заменяем им файл в `/hq-r-script/dhcp/`**

```
./hq-r-3.sh
```

**В скрипте `hr-srv-2.sh` перед запуском, меняем имя интерфейса на `ens33`**</br>
**Запускаем скрипт на HQ-SRV**
...
**В скрипте `hr-r-5.sh` перед запуском, меняем ip-адрес на `11.11.11.1`**
```
./hq-r-5.sh
```
___
 ...
 **В скрипте `hr-srv-4.sh` перед запуском, меняем номер порта `ssh` на `2020`**
...
**В скрипте `hr-r-7.sh` перед запуском, меняем имя интерфейса на `ens33` и `ssh-порт` на `2020`**
...
___
 ...
 ## **Настройка зон DNS на HQ-SRV**

**Перед запуском скрипта `hr-srv-5.sh`, нужно добавить в папку `/hq-srv-script/bind`, файл из папки `exam-2024`, под названием `local.conf`**
