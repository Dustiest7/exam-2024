...
```
cp scripts_v2.zip /root/
cd /root
unzip -e scripts_v2.zip
```
...
> [!IMPORTANT]
> Нулевой скрипт обновит полностью систему, при необходимости надо закомментировать вторую строку.</br>
> На `HQ-SRV` нужно ещё установить пакет с `nftables`.
...
**Первый скрипт конфигурирует интерфейсы, для просмотра интерфейсов на `br*` используем скрипт `nmcli.sh`**

> [!IMPORTANT]
> После запуска первого скрипта на `BR-R` и `BR-SRV`, необходимо в конфигурационных файлах интерфейсов по пути `/etc/NetworkManager/system-connections`, изменить строки с `Automatic` на `Manual`.
...
**Посмотреть содержимое отчёта `cat iperf3_logfile.txt`**
___
